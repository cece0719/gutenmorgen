							...
							<tr>
								...
								<td type="${parameterInfo['type']?html}">
									...
									<#elseif parameterInfo['type']! == 'text'>
										<#if parameterInfo['inputType']! == 'select'>
                                        	<select parameterId="${parameterInfo['id']}" type="single" onchange="$job.updateRegisterParameter('${key?html}')">
												<#list parameterInfo['selectList'] as select>
													<option>${select?html}</option>
												</#list>
											</select>
										<#elseif parameterInfo['inputType']! == 'text'>
                                            <input parameterId="${parameterInfo['id']}" type="text" onchange="$job.updateRegisterParameter('${key?html}')">
										</#if>
									</#if>
								</td>
							</tr>
...
<script type="text/javascript">
	$job = {
		...
		register : function() {
            ...
            $job.updateRegisterParameter($('#job_register')[0].processor.value);
			$('#job_register').submit();
		},
		...
		updateRegisterParameter : function(processorKey) {
			var object = {};
